...
// <editor-fold defaultstate="collapsed" desc="Boolean update(Entrancees entrancees)">
    public Boolean update(Entrancees entrancees) {
        try {
            ...
            Invocation.Builder invocationBuilder = webTarget.request(MediaType.APPLICATION_JSON);
            Response response = invocationBuilder.put(Entity.entity(entrancees, MediaType.APPLICATION_JSON));
...
// <editor-fold defaultstate="collapsed" desc=" List<Entrancees> jsonQuery(@QueryParam("query") String query , @QueryParam("sort") String sort, @QueryParam("pagenumber") Integer pageNumber, @QueryParam("rowforpage") Integer rowForPage )">

  public  List<Entrancees> jsonQuery( String query ,  String sort,
     Integer pageNumber,  Integer rowForPage ){
        ...
        try {
            ...
            client.register(authentificationProducer.httpAuthenticationFeature());
            suggestions = client
                    .target(microservicesProducer.microservicesHost() + "/autentificacion/resources/entrancees/jsonquery/")                    
  ...
  // <editor-fold defaultstate="collapsed" desc=" List<Entrancees> jsonQueryWithoutPagination( @QueryParam("query") String query , @QueryParam("sort") String sort  ){">
   
   public List<Entrancees> jsonQueryWithoutPagination( String query ,  String sort  ){
        List<Entrancees> suggestions = new ArrayList<>();
        try {
            ...
            client.register(authentificationProducer.httpAuthenticationFeature());
            suggestions = client
                    .target(microservicesProducer.microservicesHost() + "/autentificacion/resources/entrancees/jsonquerywithoutpagination/")                    
                 .queryParam("query", JmoordbDocument.encodeJson(query))
                    .queryParam("sort",JmoordbDocument.encodeJson(sort))
    ...
    public Integer countJsonQuery(String query) {
        ...
        try {
            ...
            WebTarget webTarget
                    = client.target(microservicesProducer.microservicesHost() + "/autentificacion/resources/entrancees/countjsonquery")
                             .queryParam("query", JmoordbDocument.encodeJson(query));
